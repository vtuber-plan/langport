...
def start_session(i: int, url: str, model: str, stream: bool=False, max_tokens: int=2048) -> str:
  try:
    ...
    response = openai.ChatCompletion.create(
      model=model,
      messages=[{"role": "user", "content": "Hello! What is your name?" + "a" * random.randint(1, 256)}],
      stream=stream,
...
def main(args):
  ...
  with ThreadPoolExecutor(max_workers=args.n_thread) as t:
    for i in range(args.total_task):
      task = t.submit(start_session, i=i, url=args.url, model=args.model_name, stream=False, max_tokens=args.max_tokens)
      tasks.append(task)
